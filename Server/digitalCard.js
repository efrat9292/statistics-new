...
getCardById = async (req, res) => {
    ...
    if (req.query.view) {
        ...
        let geo2 = geoip.lookup(clientIp)
        ...
        if (currentCard) {
            // currentCard.ips.ipAddress =ip;
            // currentCard.ips.ll = geo.ll;
            // console.log("hjhhj", currentCard.ips);
            //examle of specific ip
            let statistic = new Statistic();
            statistic.ipAddress = ip
            statistic.ll = geo.ll
            statistic.save((err, stat) => {
                if (err) {
                    console.log("err")
                }
                console.log("stat", stat)
            });
            currentCard.ips.push(statistic);
            //ip of current user
            let statistic1 = new Statistic();
            statistic1.ipAddress = clientIp
            statistic1.ll = geo2 ? geo2.ll : [34, 32];

            statistic1.save((err, stat) => {
                if (err) {
                    console.log("err")
                }
                console.log("stat", stat)
            });
            currentCard.ips.push(statistic1);
            // currentCard.ips.ll = geo.ll;
            console.log(currentCard.ips)

            console.log("cur9 " + currentCard.ips.ipAddress)
            console.log("cur10" + currentCard.ips.ll);
            currentCard.save()
        }
    }
        Card.findOne({ cardName: cardName, isDelete: false })
            ...
            .exec((err, card) => {
                ...
                console.log(card.socialMedia);
                console.log("ipss");
                console.log(card.ips);
    ...
    createDigitalCard = async (req, res) => {
        ...
        try {
            ...
            let statistics = card.ips.ipAddress;
